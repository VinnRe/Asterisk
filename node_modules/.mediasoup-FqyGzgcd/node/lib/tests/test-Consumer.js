...
test('transport.consume() with incompatible rtpCapabilities rejects with UnsupportedError', async () => {
    ...
    expect(router.canConsume({ producerId: audioProducer.id, rtpCapabilities: invalidDeviceCapabilities }))
        .toBe(false);
...
test('consumer.dump() succeeds', async () => {
    ...
    expect(data.consumableRtpEncodings).toEqual([
        expect.objectContaining({ ssrc: audioProducer.consumableRtpParameters.encodings?.[0].ssrc })
    ]);
    ...
    expect(data.consumableRtpEncodings[0]).toEqual(expect.objectContaining({ ssrc: videoProducer.consumableRtpParameters.encodings?.[0].ssrc }));
    expect(data.consumableRtpEncodings[1]).toEqual(expect.objectContaining({ ssrc: videoProducer.consumableRtpParameters.encodings?.[1].ssrc }));
    expect(data.consumableRtpEncodings[2]).toEqual(expect.objectContaining({ ssrc: videoProducer.consumableRtpParameters.encodings?.[2].ssrc }));
    expect(data.consumableRtpEncodings[3]).toEqual(expect.objectContaining({ ssrc: videoProducer.consumableRtpParameters.encodings?.[3].ssrc }));
